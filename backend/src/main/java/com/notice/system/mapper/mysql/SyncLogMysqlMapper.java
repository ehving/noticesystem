...
@Mapper
@DS("mysql")
public interface SyncLogMysqlMapper extends SyncLogBaseMapper {
...
SELECT
  ...
  SUM(IF(status = 'CONFLICT', 1, 0)) AS conflictCount,
  SUM(IF(status = 'ERROR', 1, 0))    AS errorCount
FROM sync_log
...
""")
    ...
    WHERE status = 'SUCCESS'
      AND update_time >= #{fromTime}
      AND (#{toTime} IS NULL OR update_time <= #{toTime})
